...
<div class="card" style="max-width:560px;margin:0 auto 16px">
    <div style="padding:20px 24px;display:flex;align-items:center;gap:14px">
        ...
        <div>
            <div style="font-size:18px;font-weight:800;color:#1e293b">Tambah Kriteria</div>
            <div style="font-size:12px;color:#64748b;margin-top:2px">Isi data kriteria penilaian baru</div>
        </div>
    </div>
</div>
...
<div style="max-width:560px;margin:0 auto">
    <div class="card">
        <div style="padding:20px 22px">
            <form method="POST" action="{{ route('kriteria.store') }}" class="kr-form" onsubmit="prosesSatuan()">
                @csrf
                ...
                <div class="mb-3" style="background:#f8fafc;border:1px solid #e2e8f0;border-radius:8px;padding:10px 14px;font-size:12px;color:#475569">
                    ...
                    <span class="badge {{ $tipe=='tetap'?'bg-info-soft':'bg-warning-soft' }}" style="font-size:11px;margin-left:4px">{{ $tipe=='tetap'?'Karyawan Tetap':'Karyawan Tidak Tetap' }}</span>
                </div>

                <div class="mb-3">
                    <label class="form-label">Nama Kriteria <span style="color:#ef4444">*</span></label>
                    <input type="text" name="nama" class="form-control @error('nama') is-invalid @enderror"
                        value="{{ old('nama') }}" placeholder="Contoh: Kehadiran" required>
                ...
                <div class="mb-3">
                    ...
                    <input type="number" name="bobot" class="form-control @error('bobot') is-invalid @enderror"
                        min="1" max="100" step="1" required placeholder="Contoh: 30" value="{{ old('bobot') }}">
                    <div style="font-size:11px;color:#64748b;margin-top:4px">
                        Total bobot semua kriteria harus = 100%.
                        Sisa bobot saat ini: <strong>{{ 100 - $totalBobot }}%</strong>
                    </div>
                ...
                <div style="background:#f8fafc;border-radius:8px;padding:12px;border:0.5px solid #e2e8f0;margin-bottom:18px">
                    <div style="display:flex;align-items:center;gap:8px;margin-bottom:6px">
                        <input type="checkbox" name="has_rentang" id="inp-has-rentang" value="1"
                            onchange="toggleRentang(this.checked)" {{ old('has_rentang') ? 'checked' : '' }}
                ...
                <div style="display:flex;gap:10px">
                    ...
                    <button type="submit" class="btn btn-primary" style="flex:2;justify-content:center;font-size:14px;padding:10px;font-weight:600">
                        <i class="ti ti-device-floppy"></i> Simpan Kriteria
                    </button>
                </div>
            </form>
        </div>
