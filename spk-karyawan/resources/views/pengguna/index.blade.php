...
<div class="card" style="margin-bottom:16px;max-width:950px;margin-left:auto;margin-right:auto">
    <div style="padding:20px 24px;display:flex;align-items:center;justify-content:space-between">
        ...
        <a href="{{ route('pengguna.create') }}" class="btn btn-primary" style="padding:8px 14px;font-size:12px;font-weight:600">
...
{{-- Search + Filter --}}
<div style="max-width:950px;margin:0 auto 12px auto">
    <form method="GET" action="{{ route('pengguna.index') }}">
        <div style="display:flex;gap:8px;flex-wrap:wrap">
            ...
            <div style="position:relative;width:150px">
                <select name="role" class="form-select" style="appearance:none;-webkit-appearance:none;padding:8px 32px 8px 12px;font-size:13px;cursor:pointer" onchange="this.form.submit()">
...
<div class="card" style="max-width:950px;margin:0 auto">
    <div class="card-header">
        <span style="font-size:16px;font-weight:700"><i class="ti ti-users"></i> Daftar Pengguna Sistem</span>
        <span style="font-size:13px;color:#64748b">{{ $pengguna->total() }} Pengguna Terdaftar</span>
    </div>
    <table class="table mb-0">
        <thead>
            <tr>
                <th style="width:40px;color:#475569;font-weight:700">No.</th>
                <th style="color:#475569;font-weight:700"><span style="display:inline-flex;align-items:center;gap:5px"><i class="ti ti-user"></i> Pengguna</span></th>
                <th style="width:110px;color:#475569;font-weight:700"><span style="display:inline-flex;align-items:center;gap:5px"><i class="ti ti-shield"></i> Role</span></th>
                <th style="width:90px;color:#475569;font-weight:700" class="text-center"><span style="display:inline-flex;align-items:center;gap:5px"><i class="ti ti-circle-check"></i> Status</span></th>
                <th style="width:90px;color:#475569;font-weight:700" class="text-center"><span style="display:inline-flex;align-items:center;gap:5px"><i class="ti ti-settings"></i> Aksi</span></th>
            </tr>
        </thead>
        <tbody>
                ...
                <td>
                    <div style="display:flex;align-items:center;gap:10px">
                        ...
                        <div>
                            ...
                            <div style="font-size:12px;color:#64748b">{{ $p->username }}</div>
                        </div>
                    </div>
                </td>
                <td>
                    <span class="badge {{ $p->role=='direktur'?'bg-info-soft':($p->role=='admin'?'bg-gray-soft':'bg-success-soft') }}" style="font-size:12px;padding:4px 11px">
                        {{ ucfirst($p->role) }}
                    </span>
                </td>
                <td class="text-center">
                    @if($nonaktif)
                    <span class="badge bg-danger-soft" style="font-size:12px;padding:4px 11px">Nonaktif</span>
                    @else
                    <span class="badge bg-success-soft" style="font-size:12px;padding:4px 11px">Aktif</span>
                    @endif
                </td>
                <td class="text-center">
                    ...
                    <a href="{{ route('pengguna.edit', $p) }}" class="btn btn-sm"
                        style="background:#2563eb;border-color:#2563eb;color:#fff;font-size:11px;font-weight:600;padding:5px 10px">
